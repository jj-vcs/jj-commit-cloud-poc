...
impl Default for TestEnvironment {
    fn default() -> Self {
        ...
        let server_url = std::env::var("JJ_TEST_SERVER_URL")
            .unwrap_or_else(|_| "http://127.0.0.1:8080".to_string());

        env.add_config(format!(
            r#"
[template-aliases]
...
[alias]
"git init" = ["cc", "init", "--server", "{server_url}"]
        "#
        ));
...
impl TestEnvironment {
    ...
    pub fn new_jj_cmd(&self) -> assert_cmd::Command {
        let jj_path = if let Ok(bin) = std::env::var("JJ_BIN") {
            std::path::PathBuf::from(bin)
        } else {
            assert_cmd::cargo::cargo_bin!("jj").to_path_buf()
        };
        ...
        if let Ok(server_url) = std::env::var("JJ_TEST_SERVER_URL") {
            cmd.env("JJ_TEST_SERVER_URL", server_url);
        }
    ...
    pub fn set_up_fake_echo_merge_tool(&self) {
        let echo_path = std::env::var("CARGO_BIN_EXE_fake-echo").unwrap_or_else(|_| "/bin/echo".to_string());
        let echo_path = to_toml_value(echo_path);
