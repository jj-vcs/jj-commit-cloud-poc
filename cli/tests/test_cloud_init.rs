...
async fn test_cloud_init_integration() {
    ...
    // 2. Execute jj cc init inside a temporary test workspace
    ...
    // 3. Assert CLI execution is successful
    cmd.assert().success();

    // 4. Verify local Jujutsu metadata directory structure
    let jj_store_path = repo_path.join(".jj/repo/store");
    assert!(jj_store_path.exists(), "The .jj/repo/store directory should exist");

    // 5. Verify backend type selection file
    ...
    // 6. Verify local Commit Cloud configuration TOML
    ...
    // 7. Verify correct parameters are serialized
