...
impl OpStoreService for CommitCloudOpStoreService {
    async fn read_operation(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "reading operations").await?;

        if let Some(op) = self.store.get_operation(&req.repo_id, &req.operation_id).await {
    ...
    async fn write_operation(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "writing operations").await?;

        let op_id = hash_operation(&op);

        self.store.put_operation(req.repo_id, op_id.clone(), op).await;
    ...
    async fn read_view(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "reading views").await?;
    ...
    async fn write_view(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "writing views").await?;

        let view_id = hash_view(&view);

        self.store.put_view(req.repo_id, view_id.clone(), view).await;
    ...
    async fn get_op_heads(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "requesting op heads").await?;
    ...
    async fn update_op_heads(
        ...
        self.ensure_repo_registered_error(&req.repo_id, "updating op heads").await?;

        let current_heads = self.store.update_op_heads(
            req.repo_id,
            &req.old_op_head_ids,
            req.new_op_head_id,
        ).await;
