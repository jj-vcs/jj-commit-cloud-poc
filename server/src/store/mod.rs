...
pub use memorystore::{CommitId, FileId, MemoryStore, OpId, ProjectId, RepoId, TreeId, ViewId};
...
pub trait Store: Send + Sync {
    ...
    // Updates op heads by removing `ids_to_remove` (if present) and appending `id_to_add`.
    // Validates that `ids_to_remove` are currently active to prevent stale client overwrites.
