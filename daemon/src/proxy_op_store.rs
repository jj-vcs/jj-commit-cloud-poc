...
impl OpStoreService for DaemonOpStoreProxy {
    ...
    async fn reconcile_op_heads(
        &self,
        request: Request<ReconcileOpHeadsRequest>,
    ) -> Result<Response<ReconcileOpHeadsResponse>, Status> {
        self.conn_manager
            .with_op_store(|mut c| async move { c.reconcile_op_heads(request).await })
            .await
    }
}
