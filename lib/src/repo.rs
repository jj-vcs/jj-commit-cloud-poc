    ...
    fn add_commit_cloud(&mut self) {
        ...
        self.add_op_store(
            ...
            Box::new(|_settings, store_path, _root_op_data| {
                let op_store = CommitCloudOpStore::load(store_path).map_err(|e| BackendLoadError(e.into()))?;
        ...
        self.add_op_heads_store(
            "commit_cloud",
            Box::new(|_settings, store_path| {
                let op_heads_store = CommitCloudOpHeadsStore::load(store_path).map_err(|e| BackendLoadError(e.into()))?;
